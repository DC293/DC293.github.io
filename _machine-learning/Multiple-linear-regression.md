...
Below we have split our car data into a 80% training and 20% test set using scipy. The random_state variable tells the function to always split the data the same way, no matter how many times we call the function. If we wanted a completely random split, we could remove it or set it to None. 
